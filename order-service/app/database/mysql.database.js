...
class MySQLDatabase {
  ...
  async connect(retryCount = 0) {
    ...
    try {
      ...
      console.log('✅ MySQL connected (order-service)');
      return this.sequelize;
    } catch (error) {
      retryCount++;
      if (retryCount >= MAX_RETRIES) throw error;
      console.warn(`⚠️ MySQL retry ${retryCount}/${MAX_RETRIES}...`);
  ...
  async disconnect() {
    if (this.sequelize) { await this.sequelize.close(); this.isConnected = false; }
  }
